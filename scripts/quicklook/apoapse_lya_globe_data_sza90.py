...
def get_dic_df(dic_iuvs, alt_lim, szaxy_lim):
    ...
    for ith,iszaxy_lim in enumerate(szaxy_lim):
        # calc data for alt_lim
        data_temp = np.where(alt_lim & iszaxy_lim, data, np.nan)
        nbin.append(np.size(data_temp[~np.isnan(data_temp)]))
        data_mean.append(np.nanmean(np.where(alt_lim & iszaxy_lim, data, np.nan)))
        sza_mean.append(np.nanmean(np.where(alt_lim & iszaxy_lim, sza, np.nan)))
        lat_mean.append(np.nanmean(np.where(alt_lim & iszaxy_lim, lat, np.nan)))
        lon_temp = np.where(alt_lim & iszaxy_lim, lon, np.nan)
        lon_mean.append(circmean(lon_temp[~np.isnan(lon_temp)], high=360, low=0))
        lt_temp = np.where(alt_lim & iszaxy_lim, lt, np.nan)
        lt_mean.append(circmean(lt_temp[~np.isnan(lt_temp)], high=24, low=0))
        efield_angle_mean.append(np.nanmean(np.where(alt_lim & iszaxy_lim, efield_angle, np.nan)))

        data_std.append(np.nanstd(np.where(alt_lim & iszaxy_lim, data, np.nan)))
        sza_std.append(np.nanstd(np.where(alt_lim & iszaxy_lim, sza, np.nan)))
        lat_std.append(np.nanstd(np.where(alt_lim & iszaxy_lim, lat, np.nan)))
        ...
        efield_angle_std.append(np.nanstd(np.where(alt_lim & iszaxy_lim, efield_angle, np.nan)))

        data_med.append(np.nanmedian(np.where(alt_lim & iszaxy_lim, data, np.nan)))
        sza_med.append(np.nanmedian(np.where(alt_lim & iszaxy_lim, sza, np.nan)))
        lat_med.append(np.nanmedian(np.where(alt_lim & iszaxy_lim, lat, np.nan)))
        efield_angle_med.append(np.nanmedian(np.where(alt_lim & iszaxy_lim, efield_angle, np.nan)))

        # calc data for alt_lim2
...
def save_globe_data_region(orbit_number, savefig=True):
    ...
    if os.path.isfile(dicpath+dname_save):
        ...
        # set lims
        alt_lim = (alt>100)&(alt<=200)
        alt_lim2 = (alt>200)&(alt<=300)
        szaxy_lim = [np.abs(sza_xy)>150, (sza_xy>=-150)&(sza_xy<-90), (sza_xy>=-90)&(sza_xy<-30),
                     (sza_xy>=-30)&(sza_xy<30), (sza_xy>=30)&(sza_xy<90), (sza_xy>=90)&(sza_xy<150)]
        dic_df = get_dic_df(dic_iuvs, alt_lim, szaxy_lim)
        dic_df2 = get_dic_df(dic_iuvs, alt_lim2, szaxy_lim)
        dic_save = {'dic_df':dic_df, 'dic_df2':dic_df2}
        ...
        if savefig:
            ...
            figpath = saveloc + 'quicklook/apoapse_l1b/Lyman-alpha/globe_data_sza90/orbit_' + '{:05d}'.format(orbit_number//100 * 100) + '/fig/'
            savename = 'orbit_' + '{:05d}'.format(orbit_number)
