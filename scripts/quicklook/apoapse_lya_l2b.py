...
import os, sys
from iuvdata import get_apoapseinfo, ApoapseSwath, SzaGeo, LocalTimeGeo, LatLonGeo
from PyUVS.graphics import H_colormap
from variables import saveloc

def quicklook_apoapse(orbit_number, linename='Lya', savefig=True):
    ...
    if len(apoinfo.files)>0:
        for ith_file, iswath_number in enumerate(apoinfo.swath_number):
            ...
            aposwath = ApoapseSwath(hdul, linename, iswath_number)
            ...
            mesh0 = aposwath.plot(ax0, flatfield_correct=True, cmap=H_colormap(), norm=mpl.colors.PowerNorm(gamma=1/2, vmin=0, vmax=30))
        ...
        ax0.set_title('Orbit ' + '{:05d}'.format(orbit_number) + ' Apoapse ' + linename)
        ...
        savepath = saveloc + 'calib/apoapse_l2b/Lyman-alpha/orbits/orbit_' + '{:05d}'.format(orbit_number//100 * 100) + '/'
...
if __name__ == '__main__':
    ...
    for iorbit_number in np.arange(n_orbit) + start_orbit:
        quicklook_apoapse(iorbit_number, linename)
        plt.close()
