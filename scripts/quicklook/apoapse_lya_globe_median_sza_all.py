...
def quicklook_apoapse_globe_median(orbit_number, orbit_half_width=5):
    path_cent = saveloc+'quicklook/apoapse_l1b/Lyman-alpha/globe_data_sza_all/orbit_{:05d}'.format(orbit_number//100 * 100)+'/npy/'
    ...
    if os.path.isfile(path_cent+fname_cent):
        ...
        for iorbit in orbit_arr:
            path = saveloc+'quicklook/apoapse_l1b/Lyman-alpha/globe_data_sza_all/orbit_{:05d}'.format(iorbit//100 * 100)+'/npy/'
        ...
        if n_img >= orbit_half_width + orbit_half_width/2:
            ...
            mesh = ax.pcolormesh(x, y, img_med, cmap=H_colormap(), norm=mpl.colors.PowerNorm(gamma=1/2, vmin=0))
            ...
            savepath = saveloc+'quicklook/apoapse_l1b/Lyman-alpha/globe_data_sza_all/orbit_{:05d}'.format(orbit_number//100 * 100)+'/median_img/npy/'
            savename = 'orbit_{:05d}'.format(orbit_number) + '.npy'
            os.makedirs(savepath, exist_ok=True)
            np.save(savepath+savename, dic_save)

            figpath = saveloc+'quicklook/apoapse_l1b/Lyman-alpha/globe_data_sza_all/orbit_{:05d}'.format(orbit_number//100 * 100)+'/median_img/fig/'
